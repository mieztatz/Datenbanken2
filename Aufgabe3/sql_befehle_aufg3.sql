...
BEGIN
   ...
   ELSIF UPDATING THEN
    IF :OLD.A_NAME != :NEW.A_NAME THEN
      SELECT z.Z_NR INTO z_nr FROM ZUORDNUNG z WHERE z.Z_KEY_OLD = CONCAT(CONCAT(:OLD.A_NAME,','),:OLD.A_VORNAME); /* --> dieses SELECT ggf. auslagern und direkt nach ELSIF UPDATING, es wird in allen IFs von Updating benötigt */
